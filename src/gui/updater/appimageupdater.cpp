...
#include "theme.h"
// SettingsDialog is apparently forward-declared somewhere, need to include the header to make the compilation work
...
private:
    explicit AppImageUpdaterShim(const QString &zsyncFileUrl, QObject *parent = nullptr)
        : QObject(parent)
        , _updater(qEnvironmentVariable(APPIMAGE_ENV_VAR).toStdString(), true)
    {
